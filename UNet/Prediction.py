...
# Load trained model
model = keras.models.load_model(r'C:\Users\Christos Tsoutsas\Desktop\University\UNet\model_x1_1.keras',
...
def plot_prediction_with_tumor(generator, model):
    # Iterate through batches to find one with tumor in the ground truth
    for index in range(len(generator)):
        X, Y = generator.__getitem__(index)
        if np.any(np.argmax(Y, axis=-1) > 0):  # Check if there is a tumor (non-zero in the mask)
            predictions = model.predict(X)
            fig, axes = plt.subplots(3, VOLUME_SLICES // 10, figsize=(20, 12))
            for i in range(VOLUME_SLICES // 10):
                axes[0, i].imshow(X[i, :, :, 0], cmap='gray')
                axes[0, i].set_title('Flair Slice')
                axes[1, i].imshow(np.argmax(Y[i], axis=-1), cmap='jet')
                axes[1, i].set_title('Ground Truth')
                axes[2, i].imshow(np.argmax(predictions[i], axis=-1), cmap='jet')
                axes[2, i].set_title('Prediction')
            plt.show()
            break  # Stop after finding and plotting the first batch with a tumor

plot_prediction_with_tumor(valid_generator, model)
